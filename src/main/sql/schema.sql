...
`parent_id` int(11) DEFAULT NULL COMMENT '父类别id  id=0时为根节点，一级类别',
...
`platform_number` varchar(200) DEFAULT NULL COMMENT '支付宝流水号',
...
`order_no` bigint(20) DEFAULT NULL COMMENT '订单号',
`user_id` int(11) DEFAULT NULL COMMENT '用户id',
`shipping_id` int(11) DEFAULT NULL COMMENT '',
`payment` decimal(20,2) DEFAULT NULL COMMENT '实际付款金额  单位-元 保留两位小数',
...
`current_unit_price` decimal(20,2) DEFAULT NULL COMMENT '生成订单时的价格 单位元 保留两位小数',
`quantity` int(10) DEFAULT NULL COMMENT '商品数量',
`total_price` decimal(20,2) DEFAULT NULL COMMENT '商品总价 单马元 保留两位小数',
`create_time` datetime DEFAULT NULL COMMENT '创建时间',
`update_time` datetime DEFAULT NULL COMMENT '更新时间',
PRIMARY KEY(`id`),
...
`receiver_district` varchar(20) DEFAULT NULL COMMENT '区',
